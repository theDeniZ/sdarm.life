...
**YouVersion note:** all calls to `api.youversion.com` are made server-side from the Cloudflare Worker — the user's IP never leaves our infrastructure. The API key is a Worker secret (`YOUVERSION_API_KEY`) and is never sent to the client bundle. Verse content is cached in Workers KV for up to 30 days.
