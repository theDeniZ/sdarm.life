...
⚠️ **`vitest` cannot go to 5.x while `apps/api` keeps this pool.** `@cloudflare/vitest-pool-workers@0.22.0` peer-requires `vitest@^4.1.0` (likewise `@vitest/runner` and `@vitest/snapshot`), so a major bump breaks the pool even though the plain-node suites in `@sdarm/types` and `@sdarm/songbook` would be fine on it.
