...
- **Passage HTML uses numeric entities, not just named ones.** DELUT Matthew 2:6 arrives as `&#34;Und du Bethlehem…&#34;` with `Juda&#39;s` inside it, so a hardcoded list of `&quot;`/`&amp;`/… leaves `&#34;` visible on the page. `decodeEntities()` in `youversion.ts` resolves decimal and hex numeric entities plus the named ones **in a single pass** — a chain of `.replace()` calls that decodes `&amp;` first would turn a literal `&amp;quot;` into a quote. Chapter text is KV-cached for 30 days, so any change here needs the `chapter` key version in `cache.ts` bumped or the fix is invisible on already-read chapters.
