...
- **`app/robots.txt/route.ts`** — Route handler that renders a shared `robots.txt` policy (prerendered static). Built via `buildRobotsTxt()` from `@sdarm/ui`. Policy allows all general crawlers, grants full access + `/llms.txt` discovery to AI answering agents (ChatGPT-User, Claude-User, etc.) with `Content-Signal` headers, and blocks training crawlers (GPTBot, ClaudeBot, etc.) and SEO/backlink crawlers (SemrushBot, etc.) entirely.
...
| `bible/BibleChapterReader` | **Client** | Default chapter view. Verse click-selection (click / Shift-range / Cmd-toggle), copy-to-clipboard with a persisted options panel (`lib/copyVerses.ts`), share link, font scale (localStorage), keyboard nav (←/→ chapters, j/k verses, g picker), presenter launch (hidden when `license.allowProjector` is false), and `BibleLicenseNotice` under the text. |
| `bible/BibleParallelReader` | **Client** | Two-column side-by-side view (`?compare=` URL param). Verses aligned by number; Psalm chapters remapped LXX↔Hebrew via `@sdarm/types` using each translation's `lxxPsalms` flag. A/B translation selects, swap, presenter launch (hidden unless **both** translations' `license.allowProjector` are true — the stricter side wins), and `BibleLicenseNotice` for both sources. |
| `bible/BiblePresenterDashboard` | **Client** | Operator view (portal): verse strip + scaled 1280×720 mirror of the display, passage picker, primary/compare translation selects, font controls, blank-screen toggle (`b`), keyboard verse navigation with Shift-range extension. Controls the display window via `BroadcastChannel('bible-projector')`. The mirror renders the same `BibleLicenseNotice` (`variant="projector"`) the real display shows, so what the operator sees matches what the room sees. |
| `bible/BibleProjector` | **Client** | Full-screen verse display. Inline fullscreen or display window (`?projector=1`, `isDisplay`). Receives `selection`/`fontScale`/`blank`/`passage`/`requestFullscreen` channel messages; sends `ready` on load. Renders single or parallel layout. `BibleLicenseNotice` (`variant="projector"`) is the grid's third row (`auto 1fr auto`) — a fixed footer, not chrome, so nothing hides it. |
| `bible/BibleProjectorOnly` | Client | Thin wrapper for `?projector=1` — mounts `BibleProjector` with `isDisplay` and closes the window on exit. This is also the one place `license.allowProjector` is enforced for the display window itself: the reader hides the button that gets here, but a hand-typed or bookmarked URL bypasses that, so this component re-checks (both sides, in parallel mode) and renders a `.bible-projector-blocked` refusal screen instead of the text when disallowed. |
| `bible/BibleLicenseNotice` | **Client** | The notice every surface showing verse text must carry — `licenseNotice()` (`notice ?? provenance`) rendered verbatim, one line per source. `variant="page"` (reader/parallel) adds a link to the license register; `variant="projector"` (display/mirror) omits it — a link on the room's screen would be a way to navigate it away from scripture mid-service. |
...
| Route | Renders |
|---|---|
| `/bible` | `PageHero` + `ContinueReadingBar` + a card per enabled translation (language chip, name, abbreviation, year, license notice via `licenseNotice()`), plus a link to `/bible/licenses`. Shows an empty-state message when the operator has enabled none. |
| `/bible/[code]` | `PageHero` + `BibleBookIndex` |
| `/bible/[code]/[book]` | Chapter number grid |
| `/bible/[code]/[book]/[chapter]` | `BibleChapterReader`; `?compare=X` → `BibleParallelReader`; `?projector=1` → `BibleProjectorOnly` (display window). JSON-LD `Article` + `generateMetadata` with canonical/hreflang; the OG/meta description appends the translation's `licenseNotice()`. |
| `/bible/licenses` | The public license register — every enabled translation's name, rights holder, license basis, verbatim `notice`, and `provenance`, read from the same `fetchTranslations()` list the landing page already fetches (no per-translation request). This is the exhibit shown to a Bible society when asking for the next license; it deliberately cannot show `permissionRef`/`permissionDate` — those are Admin-only fields on `BibleAdminTranslationDto`, not part of the public `BibleLicenseDto`. |
...
**Data source:** everything comes from our own `/api/v1/bible/*`, which serves locally-hosted public-domain texts and proxies YouVersion server-side (see [api.md](api.md#bible-content)). The browser never talks to YouVersion, and translation availability is controlled by the operator in Admin → Bible.

**URL codes** are YouVersion abbreviation slugs or local translation slugs (`/bible/delut/JHN/3`, `/bible/luther1912/JHN/3`), and book codes are USFM (`JHN`, not `JOH`). The API also accepts the raw prefixed id (`yv:51`) or a bare YouVersion number in place of the slug.
...
**License notice:** every surface that renders verse text carries `BibleLicenseNotice` — the reader and parallel view (`.bible-copyright`, under the text), and the projector/presenter mirror (`.bible-projector__notice`, `variant="projector"`, a fixed footer that nothing hides). The text is `license.notice ?? license.provenance`, rendered verbatim — never reformatted, shortened, or translated. Do not remove it from any of these surfaces.

**`license.allowProjector` is UI-only enforcement, not API enforcement.** The projector reads the same chapter route as the reader, so there is no separate endpoint to gate; an API-level check would have to trust a client-supplied "I am a projector" flag, which is not enforcement. `BibleChapterReader`/`BibleParallelReader` hide the presenter button when the translation (in parallel mode, either translation) disallows it, and `BibleProjectorOnly` re-checks and refuses to render the text if a `?projector=1` URL is reached anyway (bookmarked, hand-typed, or a stale link).
...
- [sitemap.ts](../apps/treasures/app/sitemap.ts) lists translation landings and **book indexes only** — never the chapters. Listing them came to ~2,400 URLs per translation. The chapters stay linked from each book index and stay indexable; they are simply not broadcast. `/bible/licenses` is listed too — one static URL per locale, not per translation or chapter, so it costs nothing against this budget.
- `robots.txt/route.ts` (rendered via shared `buildRobotsTxt()` from `@sdarm/ui`) disallows `?compare=` and `?projector=` — utility variants already canonicalised — and blocks training and SEO crawlers. Additionally blocks `/*/bible/*/*/` (chapter pages) for AI answering agents only; those agents are instead directed to `/llms.txt` and the compressed API endpoint (`GET /api/v1/llm/bible`), avoiding the high cost of individual chapter renders. Real search crawlers (Googlebot, bingbot) see chapters and can index them. Evidence: in a sampled day of Worker logs SemrushBot was 73% of all requests to this app and MJ12bot a further 12%, all of it inside `/bible/`, against two requests from Googlebot.
...
The app's Data Cache is backed by R2 (see [gotchas.md](gotchas.md)) so a repeat view of a chapter costs no API Worker invocations. The robots.txt policy is voluntary — crawlers that ignore it are stopped at the WAF (Cloudflare rate-limiting), which runs before the Worker and therefore costs no quota.
