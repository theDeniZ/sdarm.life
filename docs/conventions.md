...
| `SBL_URL`             | `http://localhost:3005`               | `https://sbl.sdarm.life`         | server-only |
...
`SBL_URL` is the address of the lesson's own site (`apps/sbl`). The catalogue's lesson `SectionCard` links there, and `next.config.ts` redirects `/:locale/sbl` — this app's route in v1.4.0 — to the same place. It is read in `lib/api.ts` as `SBL` for app code; `next.config.ts` reads the env var directly, because build config cannot import from `app/`.
