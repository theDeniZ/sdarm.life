...
import { expandParts, buildLineSlides, getOriginalParts, swapPrimaryLang } from '@/app/lib/format';
...
function getBgSymbolForIdx(
  ...
  if (idx === total - 1) return { symbol: 'Amen', isCenter: true, isWord: true, isRef: false };
  const p = parts[idx - 1];
  if (!p) return null;
  if (p.type === 'chorus') return { symbol: partT('chorus').slice(0, 3), isCenter: false, isWord: true, isRef: true };
...
function BgSymbol({
  ...
  const info = getBgSymbolForIdx(idx, total, parts, verseNumbers, song, partT);
  if (!info) return null;
  const cls = [
...
export default function Projector({ song, onClose, isDisplay }: Props) {
  ...
  const lineBgInfo = (() => {
    ...
    if (partType === 'chorus') return { symbol: partT('chorus').slice(0, 3), isWord: true, isRef: true };
    if (partType === 'verse') {
