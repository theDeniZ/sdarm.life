...
export default async function HomePage() {
  ...
  return (
    <>
      <PageHero
        eyebrow="Gesangbuch — Hymnarium"
        title={
          <>
            Lieder des <em>Glaubens</em>
          </>
        }
        subtitle="Heilige Gesänge aus der Tradition der Reformationsbewegung — zum Lob Gottes, zum Trost der Seele, zur Stärkung der Gemeinschaft."
          ...
          {songbooks.map((sb) => (
            <Link key={sb.id} href={`/songbooks/${sb.slug}`} className="songbook-landing-card">
              ...
              <div className="songbook-landing-card__count">
                {sb.songCount > 0 ? `${sb.songCount} Lieder` : 'Gesangbuch öffnen'}
              </div>
