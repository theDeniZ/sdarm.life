...
import { FONT_SCALES, readFontScale, writeFontScale, writeLastRead, type FontScale } from './lastRead';
...
const PILL_LABEL: Record<string, string> = {
  ...
  copyReferencePosition_top: 'Oben',
  copyReferencePosition_bottom: 'Unten',
  copyBookStyle_full: 'Voll',
  copyBookStyle_abbr: 'Kurz',
  copyBookStyle_code: 'Code',
  copyTranslationStyle_full: 'Voll',
  copyTranslationStyle_short: 'Kurz',
  copyQuoteStyle_none: '—',
  copyQuoteStyle_curly: '”…”',
  copyQuoteStyle_german: '„…”',
  copyQuoteStyle_guillemets: '«…»',
  copyLinkStyle_none: '—',
  copyLinkStyle_short: 'Kurz',
  copyLinkStyle_full: 'Voll',
  copySeparator_newline: '¶',
  copySeparator_space: '∙∙',
};

export default function BibleChapterReader({ translation, translations, books, chapter }: Props) {
  ...
  const [copyOptions, setCopyOptions] = useState<CopyOptions>(() => readCopyOptions());
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetchParallelChapter(translation.code, compareCode, chapter.book.code, chapter.chapter).then((p) => {
      if (!cancelled) setParallelChapter(p);
    });
    return () => {
      cancelled = true;
    };
  }, [compareCode, translation.code, chapter.book.code, chapter.chapter]);
  ...
  }, [locale, translation.code, prev, next, router]);
            ...
            <div className="bible-copy-options__settings">
                ...
                {(['none', 'plain', 'superscript', 'bracket'] as const).map((style) => {
                  ...
                  return (
                    <label key={style} className="bible-copy-options__radio">
                      ...
                      />
                      <span title={full}>{PILL_LABEL[key] ?? full}</span>
                    </label>
                  );
                })}
              </fieldset>
              ...
              <fieldset className="bible-copy-options__fieldset">
                ...
                {(['full', 'abbr'] as const).map((style) => {
                  const key = `copyBookStyle_${style}` as const;
                  const full = t(key);
                  return (
                    <label key={style} className="bible-copy-options__radio">
                      <input
              ...
              <fieldset className="bible-copy-options__fieldset">
                ...
                {(['full', 'short'] as const).map((style) => {
                  const key = `copyTranslationStyle_${style}` as const;
                  const full = t(key);
                  return (
                    <label key={style} className="bible-copy-options__radio">
                      <input
