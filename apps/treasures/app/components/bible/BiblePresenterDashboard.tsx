...
export default function BiblePresenterDashboard({
  ...
  useEffect(() => {
    ...
    return () => {
      // Un-blank before closing. The display window outlives this dashboard, so
      // exiting the presenter while blanked would otherwise strand the projector
      // on a black screen with no control left that could clear it. This must
      // happen here rather than in a separate effect — cleanups run in
      // definition order, and any later one would post into a closed channel.
      if (blankRef.current) ch.postMessage({ type: 'blank', on: false });
      ch.close();
    };
  }, []);
  ...
  // The un-blank on exit lives in the channel effect's cleanup above.
  useEffect(() => {
    ...
    <div className="bible-presenter">
      <div className="bible-presenter__header">
        ...
        <select
          ...
          aria-label={t('primaryTranslation')}
        >
          {translations.map((tr) => (
