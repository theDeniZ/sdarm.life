...
export default function BibleParallelReader({ translationA, translationB, translations, books, parallel }: Props) {
  ...
  // Load font + save last-read (using A as canonical)
  useEffect(() => {
    writeLastRead({
  ...
  }, [locale, prev, next, router, translationA.code, translationB.code]);
  ...
  const synthChapter: BibleChapter = useMemo(
    () => ({
      translation: { code: translationA.code, name: translationA.name },
      book: currentBook ?? {
        id: 0,
    ...
    [translationA.code, translationA.name, currentBook, parallel]
  );
  ...
  function handlePickA(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    const newChapter = remapPsalmChapter(translationA.code, newCode, parallel.bookCode, parallel.a.chapter);
