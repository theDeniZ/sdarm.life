...
/* The engine binds listeners to document and window, and writes classes and
 * custom properties onto <html> and <body>. None of that belongs to the sheet,
 * so none of it leaves with the React tree. `initSbl` hands back the function
 * that undoes it, and the shell calls that when the reader leaves.
 *
 * The module holds the live instance too. React runs an effect twice in
 * development, and a reader can leave the lesson and come back without the page
 * ever reloading — either way there must never be two engines over one sheet.
 * This used to be a flag set once for the lifetime of the module, which meant
 * the second visit got no engine at all: fresh DOM, no listeners, and a reader
 * looking at an empty sheet until they reloaded by hand. */
let disposeActive: (() => void) | null = null;

export function initSbl({ apiUrl }: { apiUrl: string }): () => void {
  disposeActive?.();

  /* Everything bound outside the sheet's own DOM goes through `on`, so teardown
   * has the list. Element-bound listeners are deliberately not registered here:
   * those elements are React's and go when it unmounts them. Listeners a drag
   * adds and removes itself are left alone for the same reason — they are gone
   * before the pointer is up. */
  const unbind: Array<() => void> = [];
  /* The overloads are what keeps `e` typed: `addEventListener('keydown', …)`
   * hands the callback a KeyboardEvent through the per-target event maps, and a
   * single signature taking an EventTarget would flatten every one of them back
   * to Event. */
  type On = AddEventListenerOptions | boolean;
  ...
  function on<K extends keyof WindowEventMap>(t: Window, type: K, fn: (e: WindowEventMap[K]) => void, opts?: On): void;
  function on<K extends keyof MediaQueryListEventMap>(
    t: MediaQueryList,
    type: K,
    fn: (e: MediaQueryListEventMap[K]) => void,
    opts?: On
  ): void;
  function on<K extends keyof VisualViewportEventMap>(
    t: VisualViewport | null,
    type: K,
    fn: (e: VisualViewportEventMap[K]) => void,
    opts?: On
  ): void;
  function on(target: EventTarget | null | undefined, type: string, fn: EventListener, opts?: On): void {
    if (!target) return;
    target.addEventListener(type, fn, opts);
    unbind.push(() => target.removeEventListener(type, fn, opts));
  }

  /* The engine sets <html lang> to the language of the lesson, which is not the
   * language of the site around it. It has to go back when the lesson does. */
  const langAtStart = document.documentElement.lang;
  ...
  /* the five presets, named — the swatch is a block of colour and has nothing
     else to be announced by */
  const THEMENAME: Dict[] = [
    { de: 'Grau', en: 'Grey', ru: 'Серый' },
    { de: 'Umschlag', en: 'Cover', ru: 'Обложка' },
    { de: 'Papier', en: 'Paper', ru: 'Бумага' },
    { de: 'Wald', en: 'Forest', ru: 'Лес' },
    { de: 'Gold', en: 'Gold', ru: 'Золото' },
  ];
  ...
  async function buildCal() {
    ...
    let h =
      '<div class="calhead"><span class="calnav" data-m="-1" title="' +
      esc(UI.prevmonth[lang]) +
      '">&lsaquo;</span><b>' +
      esc(new Intl.DateTimeFormat(LOCALE[lang], { month: 'long', year: 'numeric' }).format(first)) +
      '</b><span class="calnav" data-m="1" title="' +
      esc(UI.nextmonth[lang]) +
      '">&rsaquo;</span></div>' +
  ...
  function paintDual() {
    ...
    dualSw.setAttribute('aria-checked', dualOn_ && fits ? 'true' : 'false');
  ...
  on(document, 'pointerdown', peekDismiss, true); /* before anything opens a new one */
  on(window, 'scroll', peekDismiss, { passive: true });
  on(
    document,
    'keydown',
    function (e) {
  ...
  const UI: Phrase = {
    ...
    /* Controls whose whole content is a drawing, a chevron or a block of colour
       have no name to read out. They carry a title, which `paintControls` also
       mirrors into aria-label — and it follows the lesson's language like every
       other label, so that a Russian sheet is not announced in English. */
    tools: { de: 'Werkzeuge', en: 'Tools', ru: 'Инструменты' },
    tintown: { de: 'Eigene Farbe', en: 'Own colour', ru: 'Свой цвет' },
    prevmonth: { de: 'Voriger Monat', en: 'Previous month', ru: 'Предыдущий месяц' },
    nextmonth: { de: 'Nächster Monat', en: 'Next month', ru: 'Следующий месяц' },
    dualsw: { de: 'Zwei Sprachen nebeneinander', en: 'Two languages side by side', ru: 'Два языка рядом' },
    railsw: { de: 'Werkzeugleiste zeigen', en: 'Show the tool rail', ru: 'Показать панель инструментов' },
  ...
  function paintUI() {
    ...
    byId('railtab').setAttribute('aria-label', UI.tools[lang]);
    paintSwitch(byId('dual-sw'), UI.dualsw[lang], dualOn_);
    paintSwitch(byId('mk-rail'), UI.railsw[lang], mkRailOn);
    buildPop(); /* the swatch names are painted in the lesson's language too */
    mkPaintMode();
    mkChipPaint();
    paintFoot();
    paintControls();
  ...
  /* ——— the colophon ———
   The foot of the sheet is where the printed booklet ends, and it says the same
   things it says there: what this is, whose the content is, and what it is set
   in. It follows the language of the lesson like every other label, and it
   carries this site's name — the page it was ported from signed itself with the
   name of the person who built it and a link to his repository, which is not
   what a church's own edition should say at the bottom of every page. */
  const FOOT: Phrase = {
    kind: {
      de: 'Sabbatschullektion · im Layout des gedruckten Heftes',
      en: "Sabbath Bible Lesson · in the printed booklet's layout",
      ru: 'Урок субботней школы · в вёрстке печатного выпуска',
    },
    rights: {
      de: 'Inhalt © Gemeinschaft der Siebenten Tags Adventisten Reformationsbewegung',
      en: 'Content © Seventh Day Adventist Reform Movement',
      ru: 'Содержание © Церковь адвентистов седьмого дня реформационного движения',
    },
    bibles: {
      de: 'Bibeltext aus SDARM-Ausgaben · Luther 1912 · King James · Синодальный',
      en: 'Bible text from SDARM editions · Luther 1912 · King James · Синодальный',
      ru: 'Библейский текст из изданий SDARM · Luther 1912 · King James · Синодальный',
    },
    set: {
      de: 'Gesetzt in PT Serif & PT Sans · treasures.sdarm.life',
      en: 'Set in PT Serif & PT Sans · treasures.sdarm.life',
      ru: 'Набрано шрифтами PT Serif и PT Sans · treasures.sdarm.life',
    },
  };
  function paintFoot() {
    byId('fkind').textContent = FOOT.kind[lang];
    byId('fnote').innerHTML =
      '<a href="https://sdarm.org" target="_blank" rel="noopener">' +
      esc(FOOT.rights[lang]) +
      '</a><br>' +
      esc(FOOT.bibles[lang]);
    byId('fset').textContent = FOOT.set[lang];
  }

  /* ——— the sheet by keyboard ———
   The markup was ported from a page built entirely out of clickable <div>s: a
   mouse reaches every control on it and a keyboard reaches none. The role and
   the tab stop are applied here rather than at thirty call sites, because half
   of these controls are markup the engine writes at runtime — the language
   strip, the swatches, the calendar and the day strip are all innerHTML, and
   whatever is set on them by hand is gone the next time they are drawn.

   The names come from the `title` the engine already paints in the language of
   the lesson, mirrored into aria-label because a title alone is not announced
   by every reader. They are re-read on every paint, since the language can
   change under them.

   The two-dimensional pickers — the colour area and the hue strips — are left
   out on purpose: they are drags, and a keyboard equivalent is a slider with
   its own semantics, not a click. The preset swatches beside them carry the
   same tints and are reachable, so nothing is only available by drag. */
  const CONTROLS = [
    '#gear',
    '.lbl',
    '.zoom',
  ...
  /* The two `.sw2` toggles are the one thing on the sheet that is not a button:
     they have an on/off state, and a reader who cannot see the knob move needs
     to be told which way it went. */
  function paintSwitch(el: HTMLElement, label: string, on: boolean) {
  ...
  function paintControls() {
    document.querySelectorAll<HTMLElement>(CONTROLS).forEach(function (el) {
  ...
  /* Enter and Space are what a button answers to, and every handler on this
   page listens for a click — so that is what they are turned into. */
  on(document, 'keydown', function (e) {
    if (e.key !== 'Enter' && e.key !== ' ') return;
    const t = asElem(e.target);
    const role = t && t.getAttribute('role');
    if (role !== 'button' && role !== 'switch') return;
    e.preventDefault();
    t!.click();
  });

  /* The chrome is rebuilt by innerHTML often enough that painting it once is
   not enough. Only the chrome is watched: `#sheets` holds the lesson and can
   be a whole quarter deep when a PDF is being built, and there is nothing in
   it to give a role to. */
  let controlsQueued = false;
  const controlsObserver = new MutationObserver(function () {
  ...
  ['.bar', '#rail', '.days', '#fmtbar', '#bub'].forEach(function (sel) {
    const root = byQuery(sel);
    if (root) controlsObserver.observe(root, { childList: true, subtree: true });
  });
  ...
  function buildPop() {
    ...
    THEMES.forEach(function (t, i) {
      html +=
        ...
        i +
        '" title="' +
        esc(THEMENAME[i][lang] || THEMENAME[i].en) +
        '" style="background:' +
        t.dk +
        '"></span>';
    });
    html += '<span class="sw custom" id="swcustom" title="' + esc(UI.tintown[lang]) + '"></span>';
    tpop.innerHTML = html;
  ...
  function mkInsert(range: Range, text: string, quiet?: boolean, html?: boolean) {
    ...
    x.textContent = '×';
    x.title = T('erase');
    /* set here rather than in `paintControls`: this one lives inside the sheet,
       and the observer that repaints the chrome deliberately does not watch it */
    x.setAttribute('role', 'button');
    x.setAttribute('aria-label', T('erase'));
    x.tabIndex = 0;
  ...
  if (window.visualViewport) {
    on(window.visualViewport, 'resize', mkFmtPlace);
    on(window.visualViewport, 'scroll', mkFmtPlace);
  }
  ...
  function mkSetRail(v: boolean) {
    ...
    byId('mk-rail').setAttribute('aria-checked', v ? 'true' : 'false');
    mkPaintMode();
    if (v) {
      mkRailEdgeCalc();
      mkLinger(2400);
  ...
  on(document, 'mouseup', mkFromSelection);
  on(document, 'touchend', mkFromSelection);
  ...
  /* what counts as the reader's own hand — a programmatic scroll fires none of
     these */
  on(window, 'wheel', mkUnpin, { passive: true });
  on(window, 'touchmove', mkUnpin, { passive: true });
  ...
  on(window, 'scroll', mkScrolled, { passive: true });
  ...
  on(document, 'mouseup', mkOffer);
  on(document, 'touchend', mkOffer);
  ...
  on(document, 'mouseup', mkCallOnSel);
  on(document, 'touchend', mkCallOnSel);
  on(document, 'selectionchange', function () {
  ...
  if (!mkTouch) {
    mkRailEdgeCalc();
    on(window, 'resize', mkRailEdgeCalc);
    on(
      document,
      'pointermove',
      function (e) {
  ...
  on(matchMedia('(max-width:700px)'), 'change', function () {
  ...
  /* ——— and the way out ———
   The listeners come off, and everything the engine wrote outside the sheet is
   put back: the sheet's own DOM leaves with React, but <html> and <body> stay
   and would otherwise carry the lesson's paper grain, its tint, its rail width
   and its language into the catalogue.

   A pending load can still land after this — the fetch is not aborted, because
   the answer is worth having in the browser cache for the next visit. What it
   lands on is the detached sheet the engine captured at init, which is inert.
   `busy(false)` is the one thing it can touch out here, and it only removes a
   class that is already gone. */
  let torn = false;
  function teardown() {
    if (torn) return;
    torn = true;
    unbind.splice(0).forEach(function (off) {
      off();
    });
    controlsObserver.disconnect();
    const R = document.documentElement;
    ['--gray-dk', '--gray', '--box', '--sbl-credit', '--z', '--barh', '--railw'].forEach(function (v) {
      R.style.removeProperty(v);
    });
    R.classList.remove('paper', 'paper-grain', 'paper-dust');
    R.lang = langAtStart;
    document.body.classList.remove('loading', 'dualview', 'railon', 'writing', 'railcall', 'railout', 'bare');
  }
