...
2. **No standard emojis.** Anywhere — buttons, headers, body, errors, logs.
...
5. **Multi-column buttons where labels permit.** Don't stack short narrow buttons in one column. Examples:
   - `[ Songbooks ] [ Search ]` — two short labels in one row
   - Pagination: `[ ‹ ] [ 3/22 ] [ › ]` in one row
   - Single full-width button only for primary actions (Resume, About).
